...
@interface DBCameraCropperViewController ()
...
@implementation DBCameraCropperViewController
...
- (id)initWithImage:(UIImage *)originalImage withCropScale:(DBCameraImageScale)cropScale limitScaleRatio:(NSInteger)limitRatio {
    self = [super init];
    if (self) {
        if (cropScale==DBCameraImageScale_4x3){
            self.cropFrame = CGRectMake(0, (kScreenH-kScreenW*0.75)/2, kScreenW, kScreenW*0.75);
        }else{
            self.cropFrame = CGRectMake(0, (kScreenH-kScreenW)/2, kScreenW, kScreenW);
        }
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(DBCameraCropperDelegate)]) {
